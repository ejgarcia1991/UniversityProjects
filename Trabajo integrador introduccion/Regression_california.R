...
#Holdout method for test
californiatest<-california[18000:20640,]
california<-california[1:17999,]
...
fit3=lm(MedianHouseValue~.-Households,data=california)
summary(fit3)

fit4=lm(MedianHouseValue~MedianIncome + I(MedianIncome^2),data=california)
summary(fit4)

sqrt (sum(fit1$residuals^2 )/length(fit1$residuals ))

yprime <-predict(fit2,californiatest)
sqrt(sum(abs(californiatest$MedianHouseValue-yprime)^2)/length(yprime))

#Data entry for lab 2 R code and KNN
california <- read_csv("california/california.dat", col_names = FALSE, skip = 13)
datatypes<-scan("california/california.dat",what=character(),sep=" ",nlines=13)
names(california)<-datatypes[seq(4,30,by=3)]
