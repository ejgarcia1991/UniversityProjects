...
dts<-esl
test=sample(1:nrow(dts), 100)
train=dts[-test,]
test=dts[test,]
clases<-as.integer(unique(dts[,ncol(dts)]))

modelos<-MonotonicClassificatorModelTraining(train)
pred<-monotonicPredictUsingModels(modelos,test,clases)

pred
print(paste("Accuracy", mean(pred==test$out1)))
